pub mod guessing_game_impl {
    ...
    pub fn play_game() {
        ...
        loop {
            ...
            io::stdin()
                .read_line(&mut guess) // read the string in, if it fails, we have an error message
                .expect("Something went wrong!");

            let guess: i32 = match guess.trim().parse::<i32>() {
// try to parse the guess to an int
                Err(_) => {
// on a failed parse
...
// this is essentially a switch statement as there's no return value
                ...
                Ordering::Equal => {
                    println!(
                        "You win after {} attempts! the number was {}",
                        guesses, number
                    );
